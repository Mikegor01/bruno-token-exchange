...
export const loadToken = async (web3, networkId, dispatch) => {
  try {
    ...
    console.log(
      "Contract not deployed to current network. Please select another network with Metamask"
    );
    return null;
  }
};
...
export const loadAllOrders = async (exchange, dispatch) => {
  //Get cancel orders with cancel stream
  ...
  //Add cancelledOrders to Redux store
  dispatch(cancelledOrdersLoaded(cancelledOrders));

  //Get filled orders with Trade stream
  ...
  //Add filledOrders to Redux store
